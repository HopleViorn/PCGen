...
def sample(eval_args):
    ...
    surfZ_model.load_state_dict(torch.load('/home/ljr/Hunyuan3D-2.1/RelatedWork/BrepGen/proj_log/deepcad_ldm_surfz/epoch_1.pt'))
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            ...
            for batch_idx in range(batch_size):
                ...
                savename = f'{random_string}_{batch_idx}'
                ...
                save_control_points_as_xyz(surf_wcs_cad, os.path.join(save_folder, f'{savename}_surf_pts.xyz'))
    ...
    for batch_idx in range(batch_size):
        ...
        # Generate a unique name for the current sample
        random_string = generate_random_string(15)
        savename = f'{random_string}_{batch_idx}'

        # Save control points for visualization

        edge_wcs_cad = []
        for ncs, bbox in zip(edge_ncs_cad, edge_pos_cad):
            for pos in bbox:
                bcenter, bsize = compute_bbox_center_and_size(pos[0:3], pos[3:])
                wcs = ncs * (bsize / 2) + bcenter
                edge_wcs_cad.append(wcs)
        edge_wcs_cad = np.array(edge_wcs_cad)
        save_control_points_as_xyz(edge_wcs_cad, os.path.join(save_folder, f'{savename}_edge_pts.xyz'))
        save_control_points_as_xyz(unique_vertices, os.path.join(save_folder, f'{savename}_vert_pts.xyz'))
        ...
        # Save CAD model
        write_step_file(solid, f'{save_folder}/{savename}.step')
        write_stl_file(solid, f'{save_folder}/{savename}.stl', linear_deflection=0.001, angular_deflection=0.1)
    return 
