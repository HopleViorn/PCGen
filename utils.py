...
def get_args_ldm():
    ...
    parser.add_argument('--save_nepoch', type=int, default=1, help='number of epochs to save model')
...
def bbox_edges(corners):
    """
    Given the 8 corners of a bbox, return the 12 edges
    """
    edges = []
    # Bottom face
    edges.append(np.stack([corners[:,0], corners[:,1]]))
    edges.append(np.stack([corners[:,1], corners[:,3]]))
    edges.append(np.stack([corners[:,3], corners[:,2]]))
    edges.append(np.stack([corners[:,2], corners[:,0]]))
    # Top face
    edges.append(np.stack([corners[:,4], corners[:,5]]))
    edges.append(np.stack([corners[:,5], corners[:,7]]))
    edges.append(np.stack([corners[:,7], corners[:,6]]))
    edges.append(np.stack([corners[:,6], corners[:,4]]))
    # Connecting edges
    edges.append(np.stack([corners[:,0], corners[:,4]]))
    edges.append(np.stack([corners[:,1], corners[:,5]]))
    edges.append(np.stack([corners[:,2], corners[:,6]]))
    edges.append(np.stack([corners[:,3], corners[:,7]]))
    return np.concatenate(edges, axis=1).transpose(1,0,2)
...
def save_points_and_lines_as_ply(points, filename, bboxes=None, faces=None, condition = None):
    ...
    Args:
        points (torch.Tensor or numpy.ndarray): A tensor/array of points with shape (N, 3) or (B, N, 3) etc.
        filename (str): The output filename.
        bboxes (numpy.ndarray, optional): An array of bounding boxes with shape (N, 6). Defaults to None.
        faces (numpy.ndarray, optional): An array of faces with shape (F, 3) for mesh visualization. Defaults to None.
    """
    ...
    # Reshape to a flat list of points (N, 3)
    ...
    if bboxes is not None:
        for i, bbox in enumerate(bboxes):
            ...
            v.add_bounding_box(f'Box_{i}', position=center, size=size, color=np.array([0, 0, 255]))
