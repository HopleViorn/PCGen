...
def process(step_folder, option):
    ...
    try:
        ...
        process_furniture = option == 'furniture'

        if step_path is None:
            print(f'Skipping empty directory: {step_folder}')
            return 0
        ...
        if process_furniture:
            ...
            data_uid = os.path.splitext(os.path.basename(step_path))[0]
            sub_folder = os.path.basename(os.path.dirname(step_path))
        ...
        print(f'not saving due to error... {e}')
        return 0
...
if __name__ == '__main__':
    ...
    elif args.option == 'deepcad':
        step_dirs = load_deepcad_step(args.input) # Use deepcad loader for deepcad
        step_dirs = step_dirs[args.interval*10000 : (args.interval+1)*10000]
    else:
        step_dirs = load_abc_step(args.input, False)
        step_dirs = step_dirs[args.interval*10000 : (args.interval+1)*10000]

    # Process B-reps in parallel
    valid = 0
    process_func = partial(process, option=args.option)
    convert_iter = Pool(os.cpu_count()).imap(process_func, step_dirs)
